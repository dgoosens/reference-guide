...
If `headers` are picked, then associative array must be returned. The returned value is merged with current headers.   
If `payload` is picked, then current payload is replaced by returned value, the headers stays the same.  
